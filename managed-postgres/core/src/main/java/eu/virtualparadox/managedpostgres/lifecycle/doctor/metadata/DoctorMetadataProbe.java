...
public final class DoctorMetadataProbe {
    ...
    public DoctorProbeSnapshot inspect(final DoctorProbeRequest request) {
        ...
        if (checkedRequest.layout().isEmpty()) {
            snapshot = sections.unhealthy(
                    DoctorProbeValues.skipped(),
                    "layout is not available for metadata-backed probes");
        } else {
    ...
    private DoctorProbeSnapshot inspectWithLayout(
            final DoctorProbeRequest request,
            final PostgresLayout layout) {
        final PostgresInstanceMetadata metadata = request.metadataSnapshot().metadata().orElseThrow();
