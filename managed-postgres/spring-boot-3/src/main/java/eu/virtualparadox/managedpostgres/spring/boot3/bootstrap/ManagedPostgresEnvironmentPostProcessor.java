...
/**
 * Spring Boot 3 adapter that registers the version-agnostic managed PostgreSQL environment
 * post-processor.
 *
 * <p>Spring Boot 3's {@code org.springframework.boot.env.EnvironmentPostProcessor} differs in
 * package and type identity from the Spring Boot 4 contract, so the registration binding lives in
 * the version-specific starter while the post-processing logic stays shared in
